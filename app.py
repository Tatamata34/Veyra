...
def v21_plan_term(plan=None):
    """Human-readable plan duration used beside the plan name.
    This describes the actual plan term, e.g. 1 month, 2 months, 12 months.
    """
    months = int(getattr(plan, "months", 1) or 1) if plan is not None else 1
    if months == 12:
        return "12 months"
    return f"{months} month" if months == 1 else f"{months} months"

def v21_price_label(product, plan=None):
    """Return the storefront billing label for the actual plan.

    Product.price_label controls the billing mode. For the generic Per month /
    Per year modes, the visible suffix follows the plan duration so a 2-month
    plan is shown as /2 months rather than the misleading /month. Subscription,
    one-time and custom labels remain exactly as configured in Admin.
    """
    label = (getattr(product, "price_label", None) or "").strip()
    months = int(getattr(plan, "months", 1) or 1) if plan is not None else 1
    if not label:
        return v21_billing(months)
    if label == "one-time":
        return "one-time"
    if label == "subscription":
        return "subscription"
    if label.startswith("/"):
        # Explicit custom slash labels are respected exactly.
        return label
...
def v21_context():
    ...
    return {"v21_tr": v21_tr, "v21_json": v21_json, "v21_billing": v21_billing, "v21_price_label": v21_price_label, "v21_plan_term": v21_plan_term, "v21_meta": v21_meta, "v21_discount": v21_discount, "v21_badges": v21_badges, "v21_unread": unread}
